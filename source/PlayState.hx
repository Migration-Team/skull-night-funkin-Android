...
class PlayState extends MusicBeatState
{
	...
	public static var inSkinSelector:Bool = false;
	...
	var skinSelectMusic:FlxSound;
	var skinSelectorTxt:FlxText;
	...
	public function skinSelector():Void
	{
		...
		bg.alpha = 0;
		add(bg);
		FlxTween.tween(bg, {alpha: 0.75}, 1, {ease: FlxEase.circOut, startDelay: 0.5});

		skinSelectorTxt = new FlxText(0, -96, FlxG.width, "Skin Selector", 80);
		skinSelectorTxt.setFormat(Paths.font("vcr.ttf"), 80, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		skinSelectorTxt.scrollFactor.set();
		skinSelectorTxt.cameras = [camHUD];
		skinSelectorTxt.borderSize = 1.25;
		skinSelectorTxt.alpha = 0;
		add(skinSelectorTxt);
		...
		bfTxt.scrollFactor.set();
		bfTxt.cameras = [camHUD];
		bfTxt.borderSize = 1;
		bfTxt.alpha = 0;
		add(bfTxt);
		...
		gfTxt.scrollFactor.set();
		gfTxt.cameras = [camHUD];
		gfTxt.borderSize = 1;
		gfTxt.alpha = 0;
		add(gfTxt);
		FlxTween.tween(gfTxt, {y: gfTxt.y - 64, alpha: 1}, 0.8, {ease: FlxEase.quadOut});

		if (!ClientPrefs.lowQuality)
		{
			iconBFBG = new FlxSprite(-100, -100).loadGraphic(Paths.image('iconbackground'));
			iconBFBG.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
			iconBFBG.antialiasing = ClientPrefs.globalAntialiasing;
			iconBFBG.scrollFactor.set();
			iconBFBG.cameras = [camHUD];
			iconBFBG.updateHitbox();
			iconBFBG.screenCenter();
			iconBFBG.flipY = true;
			iconBFBG.alpha = 0;
			add(iconBFBG);
			FlxTween.tween(iconBFBG, {x: 0, y: 0, alpha: 1}, 0.8, {ease: FlxEase.quadOut});

			iconBF = new HealthIcon(boyfriend.healthIcon);
			iconBF.antialiasing = ClientPrefs.globalAntialiasing;
			iconBF.setGraphicSize(Std.int(iconBF.width * 1.5));
			iconBF.animation.curAnim.curFrame = 0;
			iconBF.scrollFactor.set();
			...
			iconBF.alpha = 0;
			add(iconBF);
			FlxTween.tween(iconBF, {x: iconBF.x + 150, y: iconBF.y + 150, alpha: 1}, 0.8, {ease: FlxEase.quadOut});

			iconGFBG = new FlxSprite(100, 100).loadGraphic(Paths.image('iconbackground'));
			iconGFBG.color = FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]);
			iconGFBG.antialiasing = ClientPrefs.globalAntialiasing;
			iconGFBG.scrollFactor.set();
			iconGFBG.cameras = [camHUD];
			iconGFBG.updateHitbox();
			iconGFBG.screenCenter();
			iconGFBG.flipX = true;
			iconGFBG.alpha = 0;
			add(iconGFBG);
			FlxTween.tween(iconGFBG, {x: 0, y: 0, alpha: 1}, 0.8, {ease: FlxEase.quadOut});

			iconGF = new HealthIcon(gf.healthIcon);
			iconGF.antialiasing = ClientPrefs.globalAntialiasing;
			iconGF.setGraphicSize(Std.int(iconGF.width * 1.5));
			iconGF.animation.curAnim.curFrame = 0;
			iconGF.y = FlxG.height - 30;
			iconGF.x = FlxG.width - 80;
			iconGF.scrollFactor.set();
			iconGF.cameras = [camHUD];
			iconGF.updateHitbox();
			iconGF.flipX = true;
			iconGF.alpha = 0;
			add(iconGF);
			FlxTween.tween(iconGF, {x: iconGF.x - 150, y: iconGF.y - 150, alpha: 1}, 0.8, {ease: FlxEase.quadOut});
		}

		if (curCharacter == 0)
			bfTxt.color = FlxColor.fromRGB(255, 255, 0);
		else if (curCharacter == 1)
			gfTxt.color = FlxColor.fromRGB(255, 255, 0);
	}
	...
	function changeSkin(change:Int = 0):Void
	{
		...
		if (curBFSkin >= bfCostumes.length)
			curBFSkin = 0;

		if (curGFSkin < 0)
			curGFSkin = gfCostumes.length-1;
		...
		case 1:
			...
			gfTxt.alpha = 0;
			gfTxt.text = gfCostumes[curGFSkin];
			gfTxt.y = (FlxG.height * 0.5) + 112;
			FlxTween.tween(gfTxt, {y: gfTxt.y + 16, alpha: 1}, 0.075);
		}
	}
	...
	function changeCharacter(change:Int = 0):Void
	{
		...
		bfTxt.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		gfTxt.color = FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !inSkinSelector && !endingSong && !inCutscene)
		{
			openChartEditor();
		}
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !inSkinSelector && !endingSong && !inCutscene) {
			persistentUpdate = false;
			paused = true;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && !paused && key > -1 && FlxG.keys.checkStatus(eventKey, JUST_PRESSED))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				else if (!inSkinSelector && canMiss) {
					noteMissPress(key);
