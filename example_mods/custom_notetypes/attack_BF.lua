--exclusive notes for bf as the playable character

function onCreate()
	--Iterate over all notes
	for i = 0, getProperty('unspawnNotes.length')-1 do
		--Check if the note is an Instakill Note
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'attack_BF' then
...
function goodNoteHit(id, direction, noteType, isSustainNote) --add note type stuff here
	if noteType == 'attack_BF' then
		characterPlayAnim('boyfriend', 'attack', true)
		playSound('defaultpunch', 1)
