--exclusive attack notes for Ms. Fortune as the playable character and opponent (to make the correct sound effect)

function onCreate()
	--Iterate over all notes
	for i = 0, getProperty('unspawnNotes.length')-1 do
		--Check if the note is an Instakill Note
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'attack_MsFortune' then
...
function opponentNoteHit(id, direction, noteType, isSustainNote)
	if noteType == 'attack_MsFortune' then
